...
const PokemonDetails = (props) => {
  const { name, isLoading, detailPokemon, fetchingPokemonDetail } = props;
  ...
  return (
    <div style={{ position: `relative`, width: `100%` }} className="card">
      {isLoading && <Loading />}
      <div className="flex flex-wrap justify-center mb-16">
        {Object.keys(detailPokemon).length > 0 && <PokemonItem {...props} />}
      </div>
    </div>
  );
};
